...
def main():
    ...
    args=ap.parse_args()
    required=['config_v2_8_0.yaml','core/family_engines.py','run_pdf_pipeline.py']
    for rel in required:
        p=ROOT/rel
    ...
    if args.pdf:
        run([sys.executable,str(ROOT/'run_pdf_pipeline.py'),'--input',args.pdf,'--all','--benchmark-reference','--engine-only','--config','config_v2_8_0.yaml'],env=env)
    print('v2.8.0 PREFLIGHT PASS')
